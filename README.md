...
公开平台支持无需注册的长期工作区。用户保存工作区 ID 与一次性显示的恢复码后，可以长期复用评估标准、文字或文件输出模板，以及由固定版本组成的业务场景。每次评估仍会生成独立的 24 小时任务快照，因此后续修改或删除资源不会改变历史任务。

工作区默认在最后访问后保留 365 天，随后进入 30 天可恢复宽限期；宽限期结束后才删除数据库记录和 `backend/data/public_workspaces` 下的文件。可通过 `PUBLIC_WORKSPACE_ACTIVE_DAYS` 与 `PUBLIC_WORKSPACE_GRACE_DAYS` 调整。生产部署必须持久化整个 `backend/data` 目录，并妥善备份；恢复码只保存哈希，服务端无法替用户找回原文。

### 支持的输出模板结构

- 文字模板：可直接描述需要的章节，也可按“字段名称：_____”逐行定义结构。
- Word：支持 `{{field_key}}` 占位符、下划线/留白区域、日期分段、单选/多选勾选项、空表格单元格，以及页眉页脚中的填写位置。
- PDF：优先填写 AcroForm 原生表单；普通文本 PDF 使用确认后的页面坐标；扫描 PDF 可生成 OCR 候选位置，但必须人工确认后才能定稿。

模板编译后会保留结构化填写位置。AI 只负责从评估证据映射内容，签名、文号、审批人等人工字段不会自动编造。缺少必填信息、文书不适用、位置失效、PDF 越界或文本溢出时，只允许生成标注了“草稿 / 待人工补充”的文件，正式定稿接口会返回阻断原因。

## 领导文稿助手运维说明

领导文稿助手仅通过私有直达地址 `/leader-assistant/` 提供服务，官网不显示入口卡片或导航。部署前在项目根目录构建前端：

```powershell
cd frontend-leadership
npm ci
npm run build
```

构建完成后，`docker compose up -d` 会将 `frontend-leadership/dist` 以只读方式挂载到 nginx；访问 `/leader-assistant` 会自动跳转至带尾斜杠的私有地址。该版本无需登录，身份档案、草稿和历史文稿仅存储在当前浏览器的本地存储中。清除浏览器数据、使用无痕模式或更换设备后无法恢复这些数据，请在需要时自行导出 Word 文稿或复制定稿内容。
